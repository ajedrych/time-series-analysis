...
testdf <- function(variable, ADF_type, ADF_max_order, BG_max_order)
{
  results_adf <- NULL
  variable <- coredata(variable[!is.na(variable)])
  
  for(augmentations in 0:ADF_max_order)
  {
    df.test_ <- adfTest(variable, lags = augmentations, type = ADF_type)
    ...
    for (order in 1:(BG_max_order+1))
    {
      bgtest_[[order]] <- bgtest(resids_~1, order = order-1)
    ...
    results_adf <- rbind(results_adf, data.frame(augmentations = augmentations, adf = df_, p_adf = p_adf,
                                                 p_bg = p_bg))
    rm(df.test_, df_, resids_, bgtest_, p_bg)
  }
